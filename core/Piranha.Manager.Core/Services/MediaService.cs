...
namespace Piranha.Manager.Services
{
    public class MediaService
    {
        ...
        public async Task<MediaListModel> GetList(Guid? folderId = null, MediaType? filter = null, int? width = null, int? height = null)
        {
            ...
            if (width.HasValue)
            {
                foreach (var mp in pairMedia.Where(m => m.media.Type == MediaType.Image || m.media.Type ==  MediaType.Rar || m.media.Type == MediaType.Zip))
                {
            ...
            model.ViewMode = model.Media.Count(m => m.Type == "Image" ||  m.Type == "Rar" ||  m.Type == "Zip") > model.Media.Count / 2 ? MediaListModel.GalleryView : MediaListModel.ListView;
